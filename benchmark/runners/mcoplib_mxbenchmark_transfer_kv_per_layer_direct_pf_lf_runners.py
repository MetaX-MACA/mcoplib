...
class Transfer_kv_per_layer_direct_pf_lf_runner(OpBenchmarkBase):
    ...
    def _prepare_data(self, dev_id):
        ...
        for _ in range(self.num_layers):
            d = torch.zeros(flat_dim, self.num_heads, self.head_dim, dtype=self.dtype, device=dev)
            dst_layers.append(d)
        ...
        return src_ptrs, dst_layers, src_indices, dst_indices

    def prepare_and_get_launcher(self, dev_id, tc_s):
        if not hasattr(torch.ops, "sgl_kernel") or not hasattr(torch.ops.sgl_kernel, "transfer_kv_per_layer_direct_pf_lf"):
             raise RuntimeError("Operator 'torch.ops.sgl_kernel.transfer_kv_per_layer_direct_pf_lf' not found.")
        with torch.cuda.stream(tc_s):
            src_ptrs, dst_layers, src_indices, dst_indices = self._prepare_data(dev_id)
        def op_launcher(*args):
            torch.ops.sgl_kernel.transfer_kv_per_layer_direct_pf_lf(
                src_ptrs, dst_layers, src_indices, dst_indices, 0, self.page_size)
    ...
    def run_verification(self, dev_id):
        if not hasattr(torch.ops, "sgl_kernel") or not hasattr(torch.ops.sgl_kernel, "transfer_kv_per_layer_direct_pf_lf"):
             return False, 1.0
        src_ptrs, dst_layers, src_indices, dst_indices = self._prepare_data(dev_id)
        torch.ops.sgl_kernel.transfer_kv_per_layer_direct_pf_lf(src_ptrs, dst_layers, src_indices, dst_indices, 0, self.page_size)
        src_layer0 = src_ptrs[0].select(1, 0)
        src_layer0_flat = src_layer0.flatten(0, 1)
        src_val = src_layer0_flat[src_indices]
        dst_val = dst_layers[0][dst_indices]
        return self.check_diff(dst_val, src_val)