...
class Transfer_kv_direct_runner(OpBenchmarkBase):
    ...
    def prepare_and_get_launcher(self, dev_id, tc_s):
        if not hasattr(torch.ops, "sgl_kernel") or not hasattr(torch.ops.sgl_kernel, "transfer_kv_direct"):
             raise RuntimeError("Operator 'torch.ops.sgl_kernel.transfer_kv_direct' not found.")
        with torch.cuda.stream(tc_s):
            src_layers, dst_layers, src_indices, dst_indices = self._prepare_data(dev_id)
        def op_launcher(*args):
            torch.ops.sgl_kernel.transfer_kv_direct(
                src_layers, dst_layers, src_indices, dst_indices, self.page_size)
    ...
    def run_verification(self, dev_id):
        if not hasattr(torch.ops, "sgl_kernel") or not hasattr(torch.ops.sgl_kernel, "transfer_kv_direct"):
             return False, 1.0
        src_layers, dst_layers, src_indices, dst_indices = self._prepare_data(dev_id)
        torch.ops.sgl_kernel.transfer_kv_direct(src_layers, dst_layers, src_indices, dst_indices, self.page_size)
        src_val = src_layers[0][src_indices]
