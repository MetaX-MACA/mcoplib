...
def run_op_benchmark(op_name):
    command = [sys.executable, TARGET_SCRIPT, "--op", op_name]
    ...
    for idx, line in enumerate(output_lines):
        ...
        if "### [0] MetaX C280" in line:
            metax_found_idx = idx

    if metax_found_idx != -1:
        start_idx = metax_found_idx + 2
        end_idx = min(metax_found_idx + 5, len(output_lines))
...
def main():
    ...
    # 获取实际需要执行的算子总数（已扣除被忽略的）
    total_ops_count = len(op_names)

    print("=" * 60)
    print(f"开始执行 Benchmark 测试 (共 {total_ops_count} 个算子)...")
    print("=" * 60)
    ...
    with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
        ...
        for op_name in op_names:
            ...
            if metax_result:
                ...
                print("  -> 未找到 MetaX C280 性能数据")
            print()

            # 文件写入
            f.write(f" -> {acc_display}  (耗时: {elapsed_time:.2f} 秒)\n")
            if metax_result:
                for line in metax_result:
                    f.write(line + "\n")
            else:
                f.write("  未找到 MetaX C280 性能数据\n")
            f.write("\n")
